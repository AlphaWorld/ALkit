...
- (void)alw_addBorderViewAtEdges:(UIRectEdge)edge color:(UIColor *)color insets:(UIEdgeInsets)insets
{
...
{
    return [objc_getAssociatedObject(self, @selector(_alw_borderEdge)) unsignedIntegerValue];
}
...
- (void)setInsetsToContainer:(UIEdgeInsets)insetsToContainer
{
    ...
    objc_setAssociatedObject(self, @selector(insetsToContainer), value, OBJC_ASSOCIATION_ASSIGN);
}
...
- (void)alw_removeAllBorderView
{
    if ([self _alw_borderEdge] != UIRectEdgeNone) {
        ...
        objc_setAssociatedObject(self, @selector(_alw_borderEdge), @(UIRectEdgeNone),
                                 OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}
...
- (NSArray *)_alw_borderViewsAtEdge:(UIRectEdge)edge
{
    ...
    if (edge & UIRectEdgeTop) {
        if ([self viewWithTag:UIRectEdgeTop + ALWborderEdgeOffset]) {
            [borders addObject:[self viewWithTag:UIRectEdgeTop + ALWborderEdgeOffset]];
        }
    }
    if (edge & UIRectEdgeBottom) {
        if ([self viewWithTag:UIRectEdgeBottom + ALWborderEdgeOffset]) {
            [borders addObject:[self viewWithTag:UIRectEdgeBottom + ALWborderEdgeOffset]];
        }
    }
    if (edge & UIRectEdgeLeft) {
        if ([self viewWithTag:UIRectEdgeLeft + ALWborderEdgeOffset]) {
            [borders addObject:[self viewWithTag:UIRectEdgeLeft + ALWborderEdgeOffset]];
        }
    }
    if (edge & UIRectEdgeRight) {
        if ([self viewWithTag:UIRectEdgeRight + ALWborderEdgeOffset]) {
            [borders addObject:[self viewWithTag:UIRectEdgeRight + ALWborderEdgeOffset]];
        }
    }
    return [borders copy];
}
